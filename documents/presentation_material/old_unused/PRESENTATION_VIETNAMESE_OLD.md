...
|------------------------------------|------|--------------|-------------------|
| [Hoàng Quý Dương]                  | [MSSV] | Team Lead / Backend Dev | Backend API, Database, Security |
| [Trần Quang Huy Anh] [Phạm Đức Trung]  | [MSSV] | Frontend Developer | React Web App, UI/UX |
| [Phạm Quang Hưng]  | [MSSV] | Mobile Developer | Flutter Mobile App |
| [Phan Ngọc Hiếu]                   | [MSSV] | Tester / Documentation | Testing, Documentation, Deployment |
...
✅ **Lập kế hoạch ngân sách** - Budget Planning
  ...
  - Cảnh báo ngưỡng 75% (Warning) và 90% (Critical) (có thể tùy chỉnh)
...
✅ **Email tự động** - Email Automation
  ...
  - Báo cáo theo lịch (hàng ngày/tuần/tháng, có thể tùy chỉnh chi tiết theo giờ, loại file)
...
│   Admin                                         │
│    │                                            │
│    └──► Quản lý users và overview tài chính     │
│                                                 │
│                                                 │
└─────────────────────────────────────────────────┘
```
...
- Profile management với avatar upload (max 2MB)
...
**Highlights:**
- 14 default categories (5 income, 9 expense) and fully customizable categories for user
...
- Multi-level warning system (Warning/Critical)
...
**Highlights:**
- 4 loại báo cáo: Monthly, Yearly, Category
...
**Highlights:**
- RBAC với 3 roles: USER, ADMIN, SUPER_ADMIN (chỉ có admin và user)
...
├──────────────────┤
│ • id (PK)        │
│ • name (UK)      │ (USER, ADMIN, SUPER_ADMIN (unused))
│ • description    │
│ • created_at     │
└──────────────────┘
```
...
- **roles**: 3 vai trò hệ thống (USER, ADMIN, SUPER_ADMIN (không sử dụng))
...
**⚠️ Issue 3: Icon pack chưa được khai thác hiệu quả**

**Vấn đề**:
- CÓ icon pack dành riêng cho app nhưng chưa được implement hoàn toàn

**Impact**: LOW - Chỉ ảnh hưởng nhỏ đến UI 

---

### SLIDE 24: NHỮNG TÍNH NĂNG ĐÃ LOẠI BỎ, HOẶC KHÔNG IMPLEMENT VÀO BẢN FINALE 

**❌ Tính năng alert cho mobile khi có transaction được thực hiện **

**Lý do không implement**:
- Để kịp tiến độ làm bản finale
- Không critical cho production

---

**❌ Super admin và tính năng xem chi tiết/xóa user **
...
**❌ CRUD Configs ở runtime cho admin **

**Lý do loại bỏ**:
- Để kịp tiến độ làm bản finale
- Không critical cho production
- Phức tạp nên có thể gây bug không cần thiết nếu không implement hoàn toàn
...
**Danh sách preferences không sử dụng** :
